...
**"따릉이는 주로 출퇴근 수단으로 쓰이는가?"** 라는 질문에 데이터를 근거로 대답합니다.

---

## 프로젝트 배경

평소 따릉이를 이용하면서 출퇴근 시간에 이용자가 몰리는 이유가 궁금했습니다.  
이용 패턴이 실제로 통근 중심인지 데이터를 통해 확인해보고자 했습니다.
...
따릉이는 출퇴근 시간대에 집중적으로 이용되는 단거리 이동 수단으로,
대중교통과 연계된 라스트마일 서비스로 기능한다.

이는 오전 8시·오후 18시 이용 피크, 평일 중심 이용 패턴, 5km 이하 단거리 이동 특성을 통해 일관되게 확인된다.

따라서 향후 지하철역 및 버스정류장 인근 대여소를 중심으로 한 수요 예측 모델 및 운영 최적화 전략으로 확장 가능하다.
---
